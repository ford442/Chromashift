...
└── engine/
    ├── graph/                # Pass-graph IR + compiler + WebGPU executor — see docs/PASS_GRAPH.md
    ...
    │   ├── altGraphs.ts               # Named graph shapes the gate can select (blur, warp)
    │   ├── exec/                      # Phase 2: walks compiled.passes and encodes the frame
    │   │   ├── plan.ts                # CompiledGraph → encodable steps (no WebGPU; owns coincidence fusion)
    │   │   ├── pool.ts                # AllocationPlan → textures, lazily; ping-pong + MSAA targets
    │   │   ├── nodePipelines.ts       # One emitted pass → bind-group layout, pipeline, uniform block
    │   │   └── WebGpuGraphExecutor.ts # The WebGPU encode path behind ?graph=1
...
**Subscription model — how a dispatch reaches the UI.** All app state is one reducer, so any state that reaches React reaches the root; three rules keep a dispatch from re-rendering the whole tree, and a change that breaks any of them re-renders everything.

1. **The reducer bails out on a no-op patch.** Every `*/patch` case goes through `patchSlice`/`withSlice` (`state/chromashiftReducer.ts`), which returns the *same* state object when nothing moved — `useReducer` compares by identity, so React skips the render entirely. This matters because several writers re-publish a value they recomputed rather than one they know changed (the luminance sampler re-reports the same rounded average ~2×/sec; image loads re-report the same aspect ratio), and without the bail-out a merely-running session re-rendered the tree a few times a second. The comparison is deliberately shallow: a patch carrying a fresh array or object is always a change, so it can never swallow a real update. See `state/reducerIdentity.test.ts`.
2. **Panels get a narrowed prop slice, not the whole bag.** `NunifOverlay` hands each panel only the keys of its own `*PanelProps` interface, via the selectors in `components/overlay/panelProps.ts`. `React.memo` compares props shallowly, so narrowing is what makes the panels' `memo()` wrappers do any work: `TracerPanel` re-renders when a tracer field changes and not when the compare layout does. The key maps there are `Record<keyof XPanelProps, true>`, so adding a field to a panel's props interface fails the build until it is listed. `components/overlay/panelProps.test.ts` asserts the isolation for every (panel, foreign key) pair.
3. **Every handler in a prop bag is reference-stable.** One unstable callback re-renders the panel holding it on every unrelated dispatch, which silently undoes (2). Concretely: the ref bundles (`hooks/refs/*`) are memoized, because dozens of `useCallback`s list `refs` as a dependency; the overlay's few action wrappers live in `components/useOverlayProps.ts` rather than being built inline in `buildOverlayProps.ts`; and handlers that need the *whole* state to do their job (serialize a preset, build a share URL, start a video or tracer export) call `store.getState()` — a stable reader over `renderStateRef` — instead of closing over `state` and listing it as a dependency. Prefer `getState()` over a `[state]` dependency for anything that ends up in a prop bag.

**Watching for regressions:** `useRenderCount` (`src/debug/renderCounts.ts`) tallies renders on `window.__renderCounts` when a session is started with `?debugRenders=1` (or `localStorage['chromashift:debugRenders'] = '1'`); it is inert otherwise. `__renderCounts.reset()` then `__renderCounts.snapshot()` brackets a window of interest. `e2e/render-churn.spec.ts` uses it to assert that an idle session re-renders *nothing*, that toggling the Perf HUD adds no re-renders outside the HUD's own subscribers, and that a tracer control re-renders `TracerPanel` alone. Note React StrictMode double-invokes render in dev, so dev counts are 2x the commit count — only zero vs. non-zero is meaningful.

**Render-loop telemetry (`engine/telemetryStore.ts`):** angles, CPU/GPU timing, frame-time history, the performance-budget flag, and collision stats are *display telemetry*, not app state — none of it is serialized into presets (see `serializeSettings.ts`'s explicit `ui` allow-list). `useAnimationLoop` used to `dispatch()` this into the reducer 5-20×/sec (unconditionally every 200 ms for angles/CPU/GPU timing, plus frame history and the budget flag whenever the Perf HUD is on, plus a 1 Hz collision-stats poll in `useAppLifecycle.useCollisionStatsPoll`), which re-rendered the *entire* component tree — including `ImageStrip`'s few-thousand-entry corpus list — on every tick, even at complete idle. It is now a small `useSyncExternalStore` store: one independent slice per telemetry kind (`renderTelemetry.setAngles/setRenderCpuTiming/setRenderGpuTiming/setFrameTimeHistory/setPerformanceBudgetExceeded/setCollisionStats`), written directly by the render loop and the collision-stats poller — no dispatch, no tree re-render. Only the few leaf components that actually display a value subscribe, via `useLiveLayerAngle(layer)` / `useRenderCpuTiming()` / `useRenderGpuTiming()` / `useFrameTimeHistory()` / `usePerformanceBudgetExceeded()` / `useCollisionStats()`; each re-renders only itself when its own slice changes (`ChromeShell`'s `TelemetryBadge`, `DiagnosticsPanel`'s `PerfHudTelemetry` + `CollisionStatsReadout`, `LayerPanel`'s `LiveAngleKnob`). `state.layers.angles` remains in the reducer as the serialized *preset baseline* angle (used by `stationaryPreview.ts`, video export, and preset export) — it is updated only by a manual knob drag, a preset load, or reset, never by the animation loop's per-frame rotation. The leaf overlay panels (`components/overlay/*Panel.tsx`) and `ImageStrip`/`PreviewStrip`/`MainViewport`/`ChromeShell` are `React.memo`-wrapped, and the "Subscription model" rules above are what make those wrappers effective: each panel receives a narrowed prop slice and every handler in it is reference-stable, so a dispatch re-renders only the panels whose own fields moved. `buildRendererState` writes into a caller-owned `RendererState` per renderer slot (`'main'`, `'compareB'`, keyed internally by `slot` — pass no `slot` for one-off callers like tests, WebXR, or offline video export) instead of allocating a fresh object graph every frame; see `buildRendererState.test.ts`'s "per-slot object reuse" tests for the reference-stability contract.
...
**CPU lanes run off the main thread.** The `wasm`/`ts` lanes' work — reading an image's pixels back via `<canvas>.getImageData()` and then classifying every pixel — used to run inline on whatever thread called `runJob()`, which for an 8K source (`8192×8192×4` ≈ 268 MB) meant a synchronous multi-hundred-millisecond stall on the thread driving `requestAnimationFrame`. This mattered precisely in the cases the CPU lanes exist to cover: `?renderer=webgl`, Firefox/Safari (no WebGPU), `?no_gpu_compute`, or a driver where the WebGPU lane declined — i.e. the fallback path janked exactly when it was the one actually running.

`useClassificationMask.ts` now backs the `CpuChoreHost` (`src/engine/compute/chores/types.ts`) with `analysisWorkerHost.ts`, which lazily spawns `src/engine/compute/analysis.worker.ts` — a module worker that owns its **own** loaded instance of the WASM engine (`loadEngine.ts`'s module-scoped state is per-realm, so the worker gets an independently-loaded copy rather than sharing the main thread's) and calls the exact same dispatch functions (`computeImageAverageLuminanceWith`, `classifyImageMaskWith`) against a transferred `ImageBitmap` + `OffscreenCanvas` instead of an `HTMLImageElement` + DOM `<canvas>` (see `PixelSource` in `wasm/imageBytes.ts`) — same math, same inputs, byte-identical mask. The worker loads lazily on the first CPU-lane job, so a healthy WebGPU session never pays for it, and `check:dist` asserts it lands in its own chunk rather than the main bundle. If the worker itself can't be used (construction throws, `createImageBitmap` throws, or it reports an error), the host falls back permanently to an in-process implementation — same thread, same math — so a job is never silently dropped; Vitest and the WASM/TS parity tests use that in-process host (`createChromashiftCpuHost` in `chromashiftHost.ts`) directly.
...
**Breadcrumbs** (Chrome-vs-Edge diagnosis): `window.gpuComputeAvailable`, `window.gpuComputeReason`, `window.gpuComputeDiagnostics` (adapter vendor/architecture/device/description, `features`, and compute `limits`), plus `window.gpuChoreBackend` / `window.gpuChoreReason` recording which lane served the last job and why the others did not. For a CPU lane, `gpuChoreBackend` is more specific than the `webgpu`/`wasm`/`ts` enum used for lane-selection logic — it reads `wasm-worker`/`ts-worker` when the analysis worker served the job, or `wasm-inline`/`ts-inline` when it ran in-process (tests, or the worker's own fallback path).

**Parity tests**: `src/engine/compute/chores/runtime.test.ts` covers the fallback order, pinned-lane behaviour, the no-silent-skip contract, and the `wasm-worker`/`ts-inline` breadcrumb labels; `src/engine/compute/chores/support.test.ts` covers detection, the kill switch, and breadcrumbs. `src/engine/compute/chores/analysisWorkerHost.test.ts` exercises the worker-backed host's request/response correlation, concurrent-request id matching, and its fallback-to-inline paths against an injected fake `Worker` (real `Worker`/`OffscreenCanvas` behavior for the worker itself is exercised by two Playwright specs instead, since Vitest's `node` environment has neither: `e2e/analysis-worker-longtask.spec.ts` for the off-main-thread claim, and `e2e/analysis-worker-mask-parity.spec.ts` for byte-identity between the worker and in-process masks). `src/engine/wasm/imageBytes.test.ts` covers the shared-canvas reuse and its per-`document` cache invalidation. `src/engine/compute/goldenMask.test.ts` checks the TS fallback against an f32-accurate port of C++ `computeClassificationMask` on a golden image (exact match, several avgLum values), asserts the WGSL `classify_band()` chain is generated from `BAND_THRESHOLDS`, and bounds the histogram-derived average within one bucket of the exact BT.709 average. `BAND_THRESHOLDS` in `src/engine/math/bandClassification.ts` is the single source of truth for band thresholds — the WGSL threshold chain is generated from it.
