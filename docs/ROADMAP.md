...
| Variable layer count (schema v7) | [#156](https://github.com/ford442/Chromashift/issues/156) | ✅ Shipped | `layers.count` (1–10) replaces the 3-tuples in state, the renderer contract and the C ABI; bind-group layouts and the coincidence compute kernel are generated from it. Executing `n` band passes is the graph-executor half — see [PASS_GRAPH.md](PASS_GRAPH.md#layer-counts) |
